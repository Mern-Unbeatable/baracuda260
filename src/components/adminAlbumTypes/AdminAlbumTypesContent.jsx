...
} from './adminAlbumTypesData';
import useAdminAlbumTypes from './useAdminAlbumTypes';
import AlbumTypeModal from './AlbumTypeModal';
...
const AlbumTypeCard = memo(({ albumType, onEdit }) => {
  ...
  return (
    <article className="flex w-full flex-col gap-[30px] rounded-[20px] border border-[rgba(0,0,0,0.16)] bg-white p-5 sm:p-8">
      <div className="flex w-full flex-col gap-4">
        <div className="flex w-full flex-col gap-7">
          <div
            className="inline-flex items-center justify-center rounded-[8px] bg-[#fde8e9] px-3 pb-[13px] pt-3"
      ...
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex min-w-0 flex-col gap-1">
          <p className="text-[14px] font-extrabold uppercase leading-[19px] tracking-[1.55px] text-[#7f8ba1]">
            {t('adminAlbumTypes.eyebrow')}
          </p>
          <h1 className="font-manrope pt-3 text-[32px] font-normal leading-[42px] tracking-[-1.68px] text-[#151e31] sm:text-[36px]">
            {t('adminAlbumTypes.title')}
          </h1>
          <p className="pt-3 text-[16px] leading-[25px] text-[#687186]">
            {t('adminAlbumTypes.subtitle')}
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex w-fit shrink-0 cursor-pointer items-center justify-center rounded-[10px] bg-[#ee1c25] px-[18px] py-[13px] text-[17px] font-bold leading-[25px] text-white shadow-[0px_5px_5px_rgba(38,99,213,0.11)] transition hover:bg-[#d41921]"
        >
