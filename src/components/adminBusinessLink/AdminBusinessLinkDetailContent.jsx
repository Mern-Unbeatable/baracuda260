    ...
    <div className="flex w-full flex-col gap-6 py-2 sm:gap-8 sm:py-4">
      ...
      <section className="flex flex-col gap-3">
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full bg-[#eef0ff] px-3 py-1 text-[12px] font-semibold uppercase tracking-wide text-[#4048cd]">
            {t(detail.typeBadgeKey)}
          </span>
          <span className="rounded-full bg-[#eff4ff] px-3 py-1 text-[12px] font-semibold uppercase tracking-wide text-[#2563eb]">
            {t(detail.categoryBadgeKey)}
          </span>
        </div>
        <h2 className="font-manrope text-[24px] font-bold leading-8 text-[#111827] sm:text-[32px] sm:leading-10">
          {t(detail.titleKey)}
        </h2>
        <p className="max-w-4xl text-[15px] leading-6 text-[#4b5563] sm:text-[16px] sm:leading-7">
          {t(detail.descriptionKey)}
        </p>
      </section>

      {/* Business link */}
      <section className="rounded-[16px] border border-[#e5e7eb] bg-white p-4 sm:p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h3 className="text-[16px] font-semibold text-[#111827]">
            {t('adminBusinessLink.detail.linkLabel')}
          </h3>
          <span className="text-[12px] text-[#9ca3af]">{t('adminBusinessLink.detail.autoGenerated')}</span>
        </div>
