...
const ChatPanel = memo(({ className = '', style, role = 'user' }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (isAdmin) {
        ...
        const { data, error } = await httpMethods.post(API_ENDPOINTS.CHAT.ROOM, {});
        if (error) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await httpMethods.get(API_ENDPOINTS.CHAT.MESSAGES(activeRoomId));
      if (error) {
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      const { data } = await httpMethods.get(API_ENDPOINTS.CHAT.MESSAGES(activeRoomId));
      if (!data) return;
  ...
  const handleSend = useCallback(async () => {
    ...
    const { data, error } = await httpMethods.post(API_ENDPOINTS.CHAT.SEND_MESSAGE(activeRoomId), {
      content: text,
    });
