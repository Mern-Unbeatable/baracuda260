...
} from '../../data/leaderboardStandings';
import { AppLink, ImgIcon, Shell, homeAsset, useMonthMenu } from '../site';
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
      <div className="flex flex-wrap items-center gap-2 sm:gap-4">
        {LEADERBOARD_ALBUM_TABS.map((tab) => {
          const active = albumTab === tab;
          return (
            <button
              key={tab}
              type="button"
              onClick={() => onAlbumTabChange(tab)}
              className={`cursor-pointer rounded-full px-4 py-2 text-[14px] font-semibold capitalize transition ${
                active
                  ? 'bg-[#4048cd] text-white'
                  : 'bg-[#f2f2f2] text-[#6b7280] hover:text-[#0d0d14]'
              }`}
            >
              {t(LEADERBOARD_ALBUM_TAB_LABEL_KEYS[tab])}
            </button>
          );
        })}
      </div>
...
const TopPhotographers = memo(({ variant = 'home' }) => {
  ...
  return (
    <section
      className={`bg-[#f7f8fa] ${
        isPage ? 'py-12 sm:py-16 xl:py-[114px]' : 'py-16 sm:py-20 xl:py-[114px]'
      }`}
    >
      <Shell>
        <div
          className={`mb-11 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between ${
            isPage ? 'gap-6 xl:mb-11' : ''
          }`}
        >
          <div>
            {!isPage ? (
            ...
            <TitleTag
              className={
                isPage
                  ? 'pt-3 text-[32px] font-bold leading-[40px] text-[#3a3a42] sm:text-[40px] sm:leading-[48px] xl:text-[48px]'
                  : 'mt-3 text-[36px] font-bold text-[#3a3a42] sm:text-[48px] sm:leading-[48px]'
              }
            >
              {isPage ? t('leaderboard.title') : t('home.leaderboard.title')}
            </TitleTag>
            <p className="mt-2 text-[16px] font-normal text-[#9ca3af]">{liveStandings}</p>
          </div>
