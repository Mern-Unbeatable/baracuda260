...
import {
  ...
  TYPE_LABEL_KEYS,
  TYPE_STYLES,
} from '@/modules/admin/data/adminCommentData';
import useAdminComment from '@/modules/admin/hooks/useAdminComment';
...
const StatusBadge = memo(({ status }) => {
  ...
  return (
    <span
      className={`inline-flex h-[22.5px] items-center gap-[5px] rounded-[99px] px-[9px] py-[3px] ${style.bg}`}
    >
      <span className={`size-[5px] shrink-0 rounded-[2.5px] ${style.dot}`} aria-hidden="true" />
      <span
...
const TypeBadge = memo(({ type }) => {
  ...
  return (
    <span
      className={`inline-flex h-5 items-center rounded-[6px] px-2 text-[11px] font-semibold leading-[16.5px] whitespace-nowrap ${style.bg} ${style.text}`}
    >
      ...
      {STATUS_FILTERS.map((filter) => {
        ...
        return (
          <button
            ...
            className={`inline-flex h-7 cursor-pointer items-center justify-center rounded-[7px] border px-[13px] py-[5px] text-[12px] font-semibold leading-[18px] transition ${
              active
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70]" role="dialog" aria-modal="true" aria-labelledby="comment-details-title">
      <button
        ...
        onClick={onClose}
      />
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-full flex-col bg-white shadow-[-8px_0_30px_rgba(15,23,42,0.12)] sm:w-[480px] sm:max-w-[480px]">
        <div className="flex items-start justify-between border-b border-[#f3f4f6] px-6 py-5">
          <div>
            ...
            <p className="text-[12px] leading-[18px] text-[#6b7280]">{row.code}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              aria-label={t('adminComment.drawer.close')}
              onClick={onClose}
              className="inline-flex size-8 cursor-pointer items-center justify-center rounded-[8px] text-[16px] text-[#6b7280] hover:bg-[#f3f4f6]"
            >
              ✕
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="relative h-[180px] overflow-hidden rounded-[12px]">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent px-4 pb-4 pt-10">
              ...
              <p className="text-[12px] leading-[18px] text-white/85">{t(row.categoryKey)}</p>
            </div>
          </div>
          ...
          <div className="pt-6">
            ...
            <div className="mt-2 rounded-[12px] border border-[#f3f4f6] bg-[#f9fafb] px-[19px] py-[14px]">
              <p className="text-[14px] leading-6 text-[#374151]">{t(row.commentKey)}</p>
            </div>
          </div>

          <div className="mt-6 rounded-[12px] border border-[#f3f4f6] p-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.55px] text-[#9ca3af]">
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-[16px] font-semibold leading-[23px] text-[#111827]">
                  {t(row.nameKey)}
                </p>
                <p className="truncate text-[12px] leading-[18px] text-[#6b7280]">{row.email}</p>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 border-t border-[#f3f4f6] pt-3.5">
              <div className="text-center">
                ...
                <p className="text-[11px] leading-[17px] text-[#9ca3af]">
      ...
      <tr className="border-b border-[#f9fafb]">
        <td className="px-5 py-[18px]">
          <input
            ...
            className="size-[15px] cursor-pointer rounded-[2px] border border-[#767676]"
          />
        </td>
        <td className="px-2 py-[18px]">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[12px] font-medium leading-[18px] whitespace-nowrap text-[#6b7280]">
              {row.code}
            </span>
          </div>
        </td>
        <td className="px-2 py-3.5">
          <div className="flex min-w-0 items-center gap-2">
            <img
              ...
              width={AVATAR_SIZE}
              height={AVATAR_SIZE}
              className="size-[30px] shrink-0 rounded-full border-2 border-[#f3f4f6] object-cover"
            />
            <div className="min-w-0">
        ...
        <td className="px-2 py-3.5">
          <div className="flex min-w-0 items-center gap-2">
            ...
            />
            <div className="min-w-0">
              <p className="truncate text-[12px] font-semibold leading-[18px] text-[#111827]">
                {t(row.photoTitleKey)}
              </p>
              <p className="truncate text-[11px] leading-[16.5px] text-[#9ca3af]">
                {t(row.categoryKey)}
        ...
        <td className="px-2 py-[22px]">
          <TypeBadge type={row.type} />
        </td>
        <td className="max-w-[240px] px-2 py-[22px]">
          <p className="truncate text-[13px] leading-[19.5px] text-[#374151]">{t(row.commentKey)}</p>
        </td>
        <td className="px-2 py-[22px] text-[12px] leading-[18px] whitespace-nowrap text-[#9ca3af]">
          {row.date}
        </td>
        <td className="px-2 py-[22px]">
          <StatusBadge status={row.status} />
          ...
          <thead>
            <tr className="border-b border-[#f3f4f6]">
              ...
              {[
                ...
                <th
                  key={column}
                  className="px-2 py-[15px] text-[11px] font-bold leading-[16.5px] tracking-[0.55px] uppercase text-[#9ca3af]"
                >
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 items-center gap-2">
                <input
                  ...
                  onChange={() => onToggleSelected(row.id)}
                  aria-label={t('adminComment.selectRow', { code: row.code })}
                  className="mt-1 size-[15px] cursor-pointer rounded-[2px] border border-[#767676]"
                />
                <img
                  src={row.avatar}
                  alt=""
...
const CommentsPagination = memo(
  ({
    ...
    return (
      <div className="flex flex-col items-center gap-3 border-t border-[#f3f4f6] px-5 py-[14px] sm:flex-row sm:justify-between">
        <p className="text-center text-[12px] leading-[18px] text-[#9ca3af] sm:text-left">
          {t('adminComment.pagination.showing', { count, total })}
        </p>
        <div
          className="flex items-center gap-1"
          role="navigation"
          aria-label={t('adminComment.pagination.aria')}
        >
          <button
            type="button"
            disabled={isFirstPage}
            aria-label={t('adminComment.pagination.previous')}
            onClick={onPrevious}
            className="inline-flex size-[30px] cursor-pointer items-center justify-center rounded-[7px] border border-[#e5e7eb] text-[12px] text-[#6b7280] disabled:cursor-not-allowed disabled:opacity-40"
          >
            ←
          </button>
          {pageNumbers.map((pageNumber) => {
            ...
            return (
              <button
                key={pageNumber}
                type="button"
                aria-label={t('adminComment.pagination.page', { page: pageNumber })}
                aria-current={active ? 'page' : undefined}
                onClick={() => onSelectPage(pageNumber)}
                className={`inline-flex size-[30px] cursor-pointer items-center justify-center rounded-[7px] border text-[12px] font-semibold leading-[18px] ${
                  active
          ...
          <button
            type="button"
            disabled={isLastPage}
            aria-label={t('adminComment.pagination.next')}
            onClick={onNext}
            className="inline-flex size-[30px] cursor-pointer items-center justify-center rounded-[7px] border border-[#e5e7eb] text-[12px] text-[#6b7280] disabled:cursor-not-allowed disabled:opacity-40"
          >
            →
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {COMMENT_STAT_CARDS.map((card) => (
        <article
          key={card.id}
          className="rounded-[14px] border border-[#f3f4f6] bg-white px-[23px] py-[21px] shadow-[0px_1px_2px_rgba(0,0,0,0.06)]"
        >
          <div className="flex items-start justify-between">
            <p className="text-[12px] font-semibold leading-[18px] tracking-[0.24px] text-[#6b7280]">
              {t(card.labelKey)}
            </p>
            <span
              className={`inline-flex size-[34px] items-center justify-center rounded-[9px] text-[16px] leading-6 ${card.iconBg}`}
              aria-hidden="true"
            >
              {card.icon}
            </span>
  ...
  return (
    <div className="flex w-full flex-col gap-6 py-2 sm:py-4">
      <header className="flex flex-col gap-1">
        <h1 className="font-manrope text-[28px] font-extrabold leading-[42px] tracking-[-0.56px] text-[#111827] sm:text-[32px]">
          {t('adminComment.title')}
        </h1>
        <p className="max-w-[640px] text-[14px] leading-[21px] text-[#6b7280]">
          {t('adminComment.subtitle')}
        </p>
      </header>
      ...
      <section
        ...
        className="overflow-hidden rounded-[16px] border border-[#f3f4f6] bg-white shadow-[0px_1px_4px_0px_rgba(0,0,0,0.06)]"
      >
        <div className="flex flex-col gap-4 border-b border-[#f3f4f6] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-[14px] font-bold leading-[21px] text-[#111827]">
