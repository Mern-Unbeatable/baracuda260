...
import {
  ...
  TOOLBAR_ICON_SIZE,
  UPLOAD_ICON_SIZE,
} from '@/modules/admin/data/adminNewsletterData';
import useAdminNewsletter from '@/modules/admin/hooks/useAdminNewsletter';
...
const RecipientOption = memo(({ option, selected, onSelect }) => {
  ...
  return (
    <button
      ...
      className={`flex w-full cursor-pointer items-center justify-between rounded-[16px] text-left transition ${
        selected
          ? 'border-2 border-[#4048cd] bg-[rgba(239,246,255,0.3)] p-[18px]'
          : 'border border-[#e2e8f0] bg-white p-[17px] hover:border-[#cbd5e1]'
      }`}
    >
      <span className="flex min-w-0 items-center">
        ...
        <span className="flex min-w-0 flex-col pl-4">
          ...
          <span className="text-[14px] leading-[21px] text-[#64748b]">{t(option.subtitleKey)}</span>
        </span>
      </span>
      {selected ? (
  ...
  return (
    <div className="flex w-full flex-col gap-6 py-2 sm:gap-6 sm:py-4">
      <header className="flex flex-col gap-1 pt-3">
        <h1 className="font-manrope text-[28px] font-medium leading-[36px] text-[#151e31] sm:text-[36px] sm:leading-[41.5px]">
          {t('adminNewsletter.title')}
        </h1>
        <p className="text-[15px] leading-6 text-[#687186] sm:text-[16px] sm:leading-[25px]">
          {t('adminNewsletter.subtitle')}
        </p>
      </header>

      <div className="flex flex-col items-stretch gap-6 xl:flex-row xl:items-start xl:gap-[38px]">
        {/* Subscriber list — natural height on mobile; capped scroll on xl only */}
        <section
          aria-label={t('adminNewsletter.tableAria')}
          className="w-full self-start overflow-hidden rounded-[12px] bg-white xl:max-w-[753px] xl:flex-1"
        >
          <div className="flex items-center justify-between rounded-t-[12px] bg-[#f6fbff] px-4">
            <div className="min-w-0 flex-1 px-2.5 py-3 text-[16px] leading-6 text-black">
              {t('adminNewsletter.columns.email')}
            </div>
            <div className="w-[140px] shrink-0 px-2.5 py-3 text-right text-[16px] leading-6 text-black sm:w-[180px] sm:text-left">
          ...
          <ul className="scrollbar-newsletter flex flex-col overflow-y-auto xl:max-h-[calc(100dvh-11rem)] xl:overflow-y-scroll">
            {subscribers.map((row) => (
              ...
              >
                ...
                <div className="w-[140px] shrink-0 px-2.5 py-4 text-right text-[14px] leading-6 text-[#0c0c0c] sm:w-[180px] sm:py-6 sm:text-left sm:text-[16px]">
                  {row.subscribedDate}
                </div>
              </li>
            ))}
        ...
        <div className="flex w-full shrink-0 flex-col gap-5 self-start xl:max-w-[768px] xl:flex-1">
          {composerOpen ? (
            <section className="flex h-auto w-full shrink-0 flex-col gap-8 overflow-hidden rounded-[24px] border border-[#e2e8f0] bg-white pb-8 pt-px">
              <div className="flex items-center justify-between border-b border-[#f1f5f9] px-5 pb-[25px] pt-6 sm:px-8">
                <div className="flex items-center gap-3">
                  <div className="flex rounded-[8px] bg-[#eef2ff] p-2">
                    <img
              ...
              <div className="flex w-full flex-col gap-6 px-5 sm:px-8">
                ...
                <div className="flex w-full flex-col gap-2">
                  ...
                  <div className="overflow-hidden rounded-[12px] border border-[#e2e8f0]">
                    ...
                    <textarea
                      ...
                      rows={6}
                      className="min-h-[160px] w-full resize-y border-0 bg-white px-4 py-4 text-[16px] leading-6 text-[#0f172a] outline-none placeholder:text-[#94a3b8]"
                    />
                  </div>
                </div>
                ...
                <div className="flex w-full flex-col gap-2">
                  ...
                  <label
                    ...
                    className="flex cursor-pointer items-center gap-4 rounded-[12px] border-2 border-dashed border-[#e2e8f0] bg-[rgba(248,250,252,0.3)] px-[18px] py-[26px] transition hover:border-[#4048cd]"
                  >
                    <span className="flex size-12 shrink-0 items-center justify-center rounded-[12px] border border-[#e2e8f0] bg-white shadow-[0px_1px_1px_rgba(0,0,0,0.05)]">
                      <img
                ...
                <div className="grid w-full grid-cols-1 gap-6 sm:grid-cols-2">
                  <label className="flex w-full flex-col gap-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-[12px] border border-[#e2e8f0] bg-white px-[17px] py-[13px] text-[16px] leading-6 text-[#334155] outline-none placeholder:text-[#94a3b8] focus:border-[#4048cd]"
                    />
                  </label>
                  <label className="flex w-full flex-col gap-2">
            ...
            <button
              ...
              className="w-full cursor-pointer rounded-[24px] border border-dashed border-[#e2e8f0] bg-white px-6 py-8 text-left text-[16px] font-semibold text-[#4048cd] transition hover:border-[#4048cd]"
            >
              {t('adminNewsletter.composer.reopen')}
            </button>
          )}

          <section className="flex h-auto w-full shrink-0 flex-col gap-8 rounded-[24px] border border-[#f1f5f9] bg-white p-5 shadow-[0px_4px_10px_rgba(0,0,0,0.05)] sm:p-[33px]">
            <div className="flex flex-col gap-[3px]">
              <h2 className="text-[20px] font-bold leading-[25px] text-[#0f172a]">
