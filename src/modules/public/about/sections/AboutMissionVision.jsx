...
const MissionVisionCard = memo(({ title, children, highlight }) => (
  <article className="flex h-full flex-col gap-5 rounded-2xl bg-[#fde8e9] p-6 sm:p-8">
    <h2 className="text-[24px] font-semibold leading-tight text-[var(--primary-text-heading-color)] sm:text-[28px]">
      {title}
    </h2>
    <div className="flex flex-1 flex-col justify-between gap-8">
      <div className="flex max-w-[560px] flex-col gap-5 text-[16px] font-normal leading-normal text-[#6b7280] sm:text-[18px]">
        {children}
      </div>
