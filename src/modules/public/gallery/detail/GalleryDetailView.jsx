...
const SignBadge = memo(({ slide, blue }) => (
  <div
    className={`absolute left-3 top-3 flex gap-2 rounded-[20px] px-4 py-1 sm:left-6 sm:top-6 sm:px-5 sm:py-[5px] ${
      blue ? 'items-center bg-[#4048cd]' : 'items-end bg-[#ee1c25]'
    }`}
  >
    <span
      className={`relative inline-flex shrink-0 items-center justify-center overflow-hidden ${
        blue ? 'size-[28px] sm:size-[35px]' : 'h-[18px] w-[20px] sm:h-[21px] sm:w-6'
            ...
            <div key={slide.id} className="flex items-center justify-between gap-1">
              <div className="flex min-w-0 items-center gap-1.5 sm:gap-2.5">
                <span
                  className={
                    blueAccent
                      ? `relative inline-flex size-[28px] shrink-0 items-center justify-center overflow-hidden sm:size-[35px] ${
                          slide.iconBoxed ? 'rounded-[4px] bg-[#4048cd]' : ''
                        }`
                      : 'inline-flex shrink-0 items-end rounded bg-[#ee1c25] px-1.5 py-1 sm:px-2.5 sm:py-[5px]'
                  }
                >
                  <img
                    src={slide.icon}
                    ...
                    className={
                      blueAccent
                        ...
                        : 'h-[16px] w-[18px] object-contain sm:h-[21px] sm:w-6'
                    }
                  />
                </span>
    ...
    >
      ...
      <section className="bg-white section-py">
        <Shell>
          ...
          <h1 className="mt-4 text-[26px] font-semibold leading-tight text-[var(--primary-text-heading-color)] sm:mt-5 sm:text-[30px] xl:text-[32px]">
            {story.title}
          </h1>
          <p className="mt-4 max-w-3xl text-[16px] font-normal leading-[26px] text-[var(--primary-text-color)] sm:text-[17px] sm:leading-[27px]">
            {story.description}
          </p>

          <div className="mt-8 grid gap-4 sm:grid-cols-2 sm:gap-5">
            {[
              { label: t('galleryDetail.votesReceived'), value: String(story.votes ?? '').replace(/,/g, '') },
              { label: t('galleryDetail.viewsCounted'), value: String(story.views ?? '').replace(/,/g, '') },
            ].map((stat) => (
              <div
                key={stat.label}
                className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-black/[0.08] bg-[#f5f5f5] px-4 py-5 text-center"
              >
                <p className="w-full text-[11px] font-semibold uppercase tracking-wider text-[#6b7280] sm:text-[12px]">
                  {stat.label}
                </p>
                <p className="w-full text-[28px] font-semibold leading-none text-[var(--primary-text-heading-color)] sm:text-[32px]">
                  {stat.value}
                </p>
              </div>
            ))}
          ...
          <div className="mt-8 flex flex-col gap-4 border-b border-black/10 pb-6 sm:mt-9 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-[12px]">
              ...
              <div>
                ...
                <p className="text-[16px] font-semibold leading-[24px] text-[var(--primary-text-heading-color)] sm:text-[18px]">
                  {story.photographer}
                </p>
              </div>
            </div>
      ...
      <section className="bg-white section-py">
        <Shell>
          <div className="overflow-hidden rounded-lg bg-[#f8fafc] p-4 sm:p-6">
            <h2 className="text-[20px] font-semibold text-[var(--primary-text-heading-color)] sm:text-[22px]">
            ...
            >
              <div className="flex flex-col gap-2">
                <label
                  ...
                  className="text-[16px] font-medium text-[var(--primary-text-heading-color)]"
                >
