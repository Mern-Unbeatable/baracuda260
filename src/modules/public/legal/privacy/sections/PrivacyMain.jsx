...
import { readLegalPage } from '@/modules/public/legal/data/legalI18n';
...
const PrivacySection = memo(({ section }) => (
  <section className="flex w-full max-w-300 flex-col gap-3">
    ...
    {section.lead ? <p className={leadClassName}>{section.lead}</p> : null}
 ...
 * Chrome from SitePageLayout; copy from i18n (`privacy`).
 */
const PrivacyContent = memo(() => {
  const { t } = useTranslation();
  const page = readLegalPage(t, 'privacy');
