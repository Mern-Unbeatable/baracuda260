    ...
    <article className="flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-[#e5e7eb] bg-white shadow-[0px_4px_8px_rgba(27,39,69,0.02)]">
      <Link to={detailPath} className="group relative block h-52 shrink-0 overflow-hidden bg-[#f3f4f6] sm:h-60">
        ...
        <span className="absolute left-3 top-3 max-w-[calc(100%-1.5rem)] rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-bold uppercase leading-3.75 tracking-[0.4px] text-[#3f51b5] sm:text-[11px]">
          {t(PREMIUM_PHOTOS_TYPE_KEYS[card.type])}
        </span>
        {card.extraPhotos ? (
          <span className="absolute bottom-3 right-3 rounded-full bg-black/65 px-2.5 py-1 text-[12px] font-semibold leading-4 text-white">
            {t('adminPremiumPhotos.extraPhotos', { count: card.extraPhotos })}
          </span>
        ) : null}
      </Link>
 ...
 * }} props
 */
const PremiumPhotosTypeFilters = memo(({ activeFilter, onFilterClick }) => {
  const { t } = useTranslation();

  return (
    <div
      role="group"
      aria-label={t('adminPremiumPhotos.filters.aria')}
      className="flex flex-wrap items-center gap-2 sm:gap-3"
    >
      {PREMIUM_PHOTOS_FILTERS.map((filter) => {
...
PremiumPhotosTypeFilters.displayName = 'PremiumPhotosTypeFilters';
...
const ShowcaseGrid = memo(({ cards }) => {
  ...
  return (
    <section
      aria-label={t('adminPremiumPhotos.gridAria')}
 ...
 */
const AdminPremiumPhotosContent = memo(() => {
  const { t } = useTranslation();
  const {
    activeFilter,
    page,
    ...
    <div className="flex w-full flex-col gap-6 sm:gap-8">
      ...
      <div className="flex justify-center pt-2 sm:pt-4">
        <AdminPagination
          ...
          navAriaLabel={t('adminPremiumPhotos.pagination.aria')}
          firstLabel={t('adminPremiumPhotos.pagination.first')}
...
AdminPremiumPhotosContent.displayName = 'AdminPremiumPhotosContent';

export default AdminPremiumPhotosContent;
