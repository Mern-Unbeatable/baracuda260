...
const cardClass =
  'rounded-[14px] border border-[#e5e7eb] bg-white p-5 sm:p-6';
...
const SectionLabel = memo(({ icon: Icon, children, tone = 'muted' }) => (
  <h2
    className={`flex items-center gap-2 text-[11px] font-bold tracking-[0.16em] ${
      tone === 'green' ? 'text-[#10b981]' : tone === 'light' ? 'text-white/55' : 'text-[#9ca3af]'
    }`}
  >
...
const ContactRow = memo(({ icon: Icon, label, value }) => (
  <div className="flex items-start gap-2.5">
    <Icon size={15} className="mt-0.5 shrink-0 text-[#9ca3af]" aria-hidden="true" />
    <div className="min-w-0">
      <p className="text-[10px] font-bold tracking-[0.14em] text-[#9ca3af]">{label}</p>
...
const TimelineStep = memo(({ step, title, pendingTitle, pendingMeta, transitNote, isLast }) => {
        ...
        <div className="flex items-start justify-between gap-3">
            ...
            {isPending ? (
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-[14px] font-semibold text-[#9ca3af]">
                  {pendingTitle}{' '}
                  <span className="font-medium text-[#b0b7c6]">{pendingMeta}</span>
                </p>
                {transitNote ? (
          ...
          {!isPending && step.date ? (
            <div className="shrink-0 text-right">
              <p className="text-[12px] font-medium text-[#6b7280]">{step.date}</p>
    ...
    <div className="mx-auto flex w-full max-w-395 flex-col gap-5">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              onClick={handleCopy}
              aria-label={t('storeOrders.copyAria', { number: order.orderNumber })}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <section className={`${cardClass} flex flex-col`}>
          <SectionLabel icon={CreditCard}>{t('storeOrders.detail.payment')}</SectionLabel>
          <p className="mt-5 flex items-center gap-2.5 text-[16px] font-bold text-[#111827]">
            <span className="size-2.5 shrink-0 rounded-full bg-[#10b981]" aria-hidden="true" />
            {t('storeOrders.detail.paid')}
          </p>
        </section>
        <section className={`${cardClass} flex flex-col`}>
          <SectionLabel icon={Package}>{t('storeOrders.detail.fulfillment')}</SectionLabel>
      ...
      <section className={cardClass}>
        <SectionLabel icon={UserRound}>{t('storeOrders.detail.customerInfo')}</SectionLabel>
        <div className="mt-5 flex items-center gap-3">
          ...
          <p className="text-[16px] font-bold text-[#111827]">{order.customerName}</p>
        </div>
      ...
      <section className={cardClass}>
        <SectionLabel icon={Package}>{t('storeOrders.detail.productInfo')}</SectionLabel>
        <div className="mt-5 flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-start gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] text-[#6b7280]">{order.productMeta}</p>
          ...
          <div className="shrink-0 text-right">
            ...
            <p className="mt-1 text-[18px] font-bold text-[#111827]">{order.total}</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[14px] border border-[#a7f3d0] bg-white p-5 sm:p-6">
        ...
        <div className="mt-5 flex flex-col gap-3 text-[14px]">
          <div className="flex items-center justify-between gap-3">
            <span className="text-[#6b7280]">{t('storeOrders.detail.orderTotal')}</span>
            <span className="font-bold text-[#111827]">{order.total}</span>
          </div>
          <div className="flex items-center justify-between gap-3">
            <span className="text-[#6b7280]">{t('storeOrders.detail.commission')}</span>
            <span className="font-medium text-[#9ca3af]">{order.commission}</span>
          </div>
        </div>
        <div className="mt-5 flex items-center justify-between gap-3 rounded-[10px] bg-[#00c853] px-4 py-3.5 text-white">
          <span className="text-[14px] font-semibold">{t('storeOrders.detail.sellerEarnings')}</span>
          <span className="text-[22px] font-extrabold leading-none">{order.earnings}</span>
        </div>
      ...
      <section className={cardClass}>
        <SectionLabel icon={Clock}>{t('storeOrders.detail.orderTimeline')}</SectionLabel>
        <div className="mt-5">
          {order.timeline.map((step, index) => (
            <TimelineStep
              ...
              transitNote={
                step.id === 'delivered' && step.state === 'pending' ? order.transitNote : null
              }
