  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col gap-6">
            ...
            <ul className="flex flex-col gap-2.5" role="list">
              {tier.featureKeys.map((featureKey) => (
                <li
                  key={featureKey}
                  className="flex items-center gap-2 pt-0 sm:pt-2"
                >
                  <Check
                    size={13}
                    ...
                    strokeWidth={3}
                  />
                  <span className="text-[14px] leading-5 text-[#111827]">
                    {t(featureKey)}
                  </span>
                </li>
              ))}
      ...
      {href ? (
        ...
        <Button unstyled type="button" className={ctaClassName}>
          {t('uploadPhotos.enterNow')}
          <ArrowRight size={16} aria-hidden="true" />
        </Button>
      )}
...
const UploadPhotosContent = memo(
  ({ routeSet = UPLOAD_ROUTE_SETS.standalone }) => {
    const { t } = useTranslation();
